...
//	private static final String EMPTY_CELL = CELL_TXT;
	...
	protected String consoleCell(String celStr) {
		...
		if (celStr.equals(Messages.EMPTY)) {
			consoleStr = EMPTY_CELL;
		} else if (celStr.equals(Messages.LAND)) {
			consoleStr = LAND_CELL;
		} else if (celStr.equals(Messages.EXIT_DOOR)) {
			consoleStr = EXIT_CELL;
		} else if (celStr.contains(Messages.EXIT_DOOR)) {
			consoleStr = exitCel(celStr);
		} else if (celStr.equals(Messages.BOX)) {
			consoleStr = BOX_CELL;
		} else if (celStr.equals(Messages.EMPTY_BOX)) {
			consoleStr = EMPTY_BOX;
		} else {
			consoleStr = characterCell(celStr);
		}
	...
	private static String characterColor(String str) {
		...
		if (str.contains(Messages.GOOMBA)) {
			colorStr = GOOMBA_COLOR;
		} else if (str.contains(Messages.MARIO_STOP) ||
				 str.contains(Messages.MARIO_LEFT) ||
				 str.contains(Messages.MARIO_RIGHT)  ) {
			colorStr = MARIO_COLOR;
		} else if (str.contains(Messages.MUSHROOM)) {
			colorStr = MUSHROOM_COLOR;
		}
